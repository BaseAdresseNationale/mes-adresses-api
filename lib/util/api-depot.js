...
function sendPinCode(habilitationId) {
  return got.post(`${API_BAN_URL}/habilitations/${habilitationId}/authentication/email/send-pin-code`, {
    headers: {
      authorization: `Token ${process.env.API_BAN_TOKEN}`,
    }
  })
...
async function validatePinCode(habilitationId, code) {
  const {body} = await got.post(`${API_BAN_URL}/habilitations/${habilitationId}/authentication/email/validate-pin-code`, {
    headers: {
      authorization: `Token ${process.env.API_BAN_TOKEN}`,
    },
    json: {code},
