...
class Mongo {
  ...
  async getNumerosCount(model, idProperty) {
    const nbNumeros = await this.db.collection('numeros').countDocuments({[idProperty]: this.parseObjectID(model._id)})
    const nbNumerosCertifies = await this.db.collection('numeros').countDocuments({[idProperty]: this.parseObjectID(model._id), certifie: true})
