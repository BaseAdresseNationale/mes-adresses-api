...
// Returns the same expression as $set (object) but adapted to the expression $and (array)
function getAndExpression(setExpression) {
  const {$set} = setExpression
  const andExpression = []
...
const batchUpdateSchema = Joi.object().keys({
  ...
  comment: Joi.string().max(5000).allow(null),
  toponyme: Joi.string().custom(validObjectID).allow(null),
  certifie: Joi.boolean().allow(null)
})
...
async function batchUpdateNumeros(idBal, body) {
  ...
  const numeros = numerosIds.map(id => {
    if (!id) {
      throw createError(400, 'Id can’t be null')
    }

    return mongo.parseObjectID(id)
  })

  // Corresponding to $or expression.
  // For all request, we search different voie than the one coming from the changes.
  const orExpression = [
    {voie: {$ne: voie}}
  ]

  // Corresponding to $set expression.
  const setExpression = {$set: {
    voie,
    _updated: now
  }}

  // The following conditions add requests to $set and $or expressions according to changes

  if (certifie !== undefined) {
    orExpression.push({
      certifie: {$ne: Boolean(certifie)}
    })
    setExpression.$set.certifie = Boolean(certifie)
  }

  if (positionType) {
    orExpression.push({'positions.0.type': {$ne: positionType}})
    setExpression.$set['positions.0.type'] = positionType
  }

  if (toponyme !== undefined) {
    orExpression.push({toponyme: {$ne: toponyme}})
    setExpression.$set.toponyme = toponyme
  }

  if (comment !== undefined) {
    orExpression.push({comment: {$ne: comment}})
    setExpression.$set.comment = comment
  }
  ...
  if (modifiedCount > 0) {
    const updatedVoies = await mongo.db.collection('numeros').distinct('voie', {
      _id: {$in: numeros},
      _bal: mongo.parseObjectID(idBal),
      $and: getAndExpression(setExpression)
    })
    const voieIds = updatedVoies.map(id => id)
...
async function batchRemoveNumeros(idBal, body) {
  ...
  const numeros = numerosIds.map(id => mongo.parseObjectID(id))

  const voies = await mongo.db.collection('numeros').distinct('voie', {
    _id: {$in: numeros}
  })

  const voieIds = voies.map(id => id)
