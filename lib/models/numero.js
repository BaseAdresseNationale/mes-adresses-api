...
const createSchema = Joi.object().keys({
  ...
  suffixe: Joi.string().regex(/^[a-z0-9]+$/i).max(10).allow(null),
...
function displaySuffixe(suffixe) {
  if (suffixe) {
    return isNaN(suffixe) ? (
      suffixe.trim()
    ) : (
      '-' + suffixe.trim()
    )
...
function expandModel(numero) {
  return {
    ...numero,
    numeroComplet: numero.numero + displaySuffixe(numero.suffixe)
  }
}
