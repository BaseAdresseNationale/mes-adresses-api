...
async function updateNumerosByBatch(idBal, body) {
  ...
  const voiesBeforeUpdate = await mongo.db.collection('numeros').distinct('voie', {_id: {$in: formattedNumerosIds}, certifie: {$eq: certifie}})
  ...
  if (modifiedCount) {
    const voiesAfterUpdate = await mongo.db.collection('numeros').distinct('voie', {_id: {$in: formattedNumerosIds}, certifie: {$eq: certifie}})
