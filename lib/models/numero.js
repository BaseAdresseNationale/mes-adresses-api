...
const createSchema = Joi.object().keys({
  ...
  suffixe: Joi.string().regex(/^[a-z][a-z0-9]*$/i).max(10).allow(null),
...
async function create(idVoie, payload) {
  ...
  numero.suffixe = numero.suffixe ? numero.suffixe.toLowerCase() : null
...
async function update(id, payload) {
  ...
  numeroChanges.suffixe = numeroChanges.suffixe ? numeroChanges.suffixe.toLowerCase() : null
...
function expandModel(numero) {
  return {
    ...numero,
    numeroComplet: `${numero.numero}${numero.suffixe ? numero.suffixe.trim() : ''}`
  }
}
