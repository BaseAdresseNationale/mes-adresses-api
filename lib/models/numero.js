...
function formatIdsToObjectIds(idsList) {
  const objectIdsList = []
  idsList.forEach(id => {
    objectIdsList.push(mongo.parseObjectID(id))
  })

  return objectIdsList
}

async function updateNumerosByBatch(idBal, body) {
  const now = new Date()
  const {numeros, certifie} = body
  const formattedNumerosIds = formatIdsToObjectIds(numeros)
  const numerosList = await mongo.db.collection('numeros').find({_id: {$in: formattedNumerosIds}}).toArray()
  const voiesIds = uniqBy(numerosList.map(n => n.voie), voieId => voieId.toString())
  const formattedVoieIds = formatIdsToObjectIds(voiesIds)

  await mongo.db.collection('numeros').updateMany(
    {_id: {$in: formattedNumerosIds}},
    {$set: {certifie, _updated: now}}
  )
