...
async function fetchOne(id) {
  return mongo.db.collection('numero').findOne({_id: mongo.parseObjectID(id)})
}

async function fetchAll(idVoie) {
  return mongo.db.collection('numeros').find({voie: idVoie})
}

async function remove(id) {
  await mongo.db.collection('numero').deleteOne({_id: mongo.parseObjectID(id)})
}
