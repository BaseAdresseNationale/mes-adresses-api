...
async function create(baseLocale, payload) {
  ...
  voie.nomAlt = voie.nomAlt && Object.keys(voie.nomAlt).length > 0 ? voie.nomAlt : null
...
async function importMany(idBal, rawVoies, options = {}) {
  ...
  const voies = rawVoies
    .map(v => {
      ...
      voie.nomAlt = v.nomAlt && Object.keys(v.nomAlt).length > 0 ? v.nomAlt : null
...
async function update(id, payload) {
  ...
  const nomAlt = await addNomAlt('voies', id, voieChanges.nomAlt)

  if (nomAlt) {
    voieChanges.nomAlt = nomAlt
  }
