...
async function getNumerosCount(idVoie) {
  const nbNumeros = await mongo.db.collection('numeros').countDocuments({voie: mongo.parseObjectID(idVoie)})
  const nbNumerosCertifies = await mongo.db.collection('numeros').countDocuments({voie: mongo.parseObjectID(idVoie), certifie: true})
  const isAllCertified = nbNumeros > 0 && nbNumeros === nbNumerosCertifies

  return {nbNumeros, nbNumerosCertifies, isAllCertified}
}

async function getExpandedVoie(voie) {
  const {nbNumeros, nbNumerosCertifies, isAllCertified} = await getNumerosCount(voie._id)

  voie.nbNumeros = nbNumeros
  voie.nbNumerosCertifies = nbNumerosCertifies
  voie.isAllCertified = isAllCertified
