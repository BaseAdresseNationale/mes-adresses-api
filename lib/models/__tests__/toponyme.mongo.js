...
test('create a Toponyme', async t => {
  ...
  const toponyme = await Toponyme.create(_id, '12345', {nom: 'foo', certifie: false})

  const keys = ['_id', '_bal', 'nom', 'positions', 'parcelles', 'certifie', 'commune', '_created', '_updated']
...
test('create a Toponyme / with positions, parcelles & certifie to true', async t => {
  const _id = new mongo.ObjectID()
...
test('importMany', async t => {
  ...
  t.deepEqual(
    pick(v1, 'nom', 'commune', 'positions', 'parcelles', 'certifie', '_updated', '_created'),
    pick(toponyme1, 'nom', 'commune', 'positions', 'parcelles', 'certifie', '_updated', '_created')
  )

  const v2 = toponymes.find(v => v.nom === 'bar')
  t.deepEqual(
    pick(v2, 'nom', 'commune', 'positions', 'parcelles', 'certifie'),
    pick(toponyme2, 'nom', 'commune', 'positions', 'parcelles', 'certifie')
  )
})
