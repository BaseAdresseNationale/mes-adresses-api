...
test('valid payload update: with lieuDit', t => {
  const _idVoie = new mongo.ObjectID()
...
test('invalid payload update: invalid lieuDit', t => {
  const _idVoie = new mongo.ObjectID()
  ...
  const {error} = updateSchema.validate(numero)
  t.true(error.message.includes('"lieuDit" failed custom validation'))
})
