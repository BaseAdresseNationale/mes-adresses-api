...
test('create a numero', async t => {
  ...
  const keys = ['_id', '_bal', 'commune', '_updated', '_created', 'voie', 'numero', 'suffixe', 'comment', 'positions', 'lieuDit']
...
test('create a numero with lieuDit', async t => {
  const idBal = new mongo.ObjectID()
  ...
  const numero = await Numero.create(idVoieA, {numero: 42, lieuDit: idVoieB.toString()})

  const keys = ['_id', '_bal', 'commune', '_updated', '_created', 'voie', 'numero', 'suffixe', 'positions', 'comment', 'lieuDit']
  t.true(keys.every(k => k in numero))
  t.is(Object.keys(numero).length, keys.length)
})

test('create a numero with lieuDit / no voie positions', async t => {
  const idVoieA = new mongo.ObjectID()
  const idVoieB = new mongo.ObjectID()
  await mongo.db.collection('voies').insertOne({
    _id: idVoieA,
  ...
  await t.throwsAsync(Numero.create(idVoieA, {numero: 42, lieuDit: idVoieB.toString()}), {message: 'Lieu-dit not found'})
})
...
test('update a numero / add lieuDit', async t => {
  const idBal = new mongo.ObjectID()
  const idVoieA = new mongo.ObjectID()
  const idVoieB = new mongo.ObjectID()
  const _id = new mongo.ObjectID()
  ...
  t.deepEqual(numero.lieuDit, idVoieB)
})

test('update a numero / change lieuDit', async t => {
  const idBal = new mongo.ObjectID()
  const idVoieA = new mongo.ObjectID()
  const idVoieB = new mongo.ObjectID()
  const idVoieC = new mongo.ObjectID()
  const _id = new mongo.ObjectID()
  const referenceDate = new Date('2019-01-01')
  const positionA = {
    point: {type: 'Point', coordinates: [1, 1]},
  ...
  t.deepEqual(numero.lieuDit, idVoieC)
})
