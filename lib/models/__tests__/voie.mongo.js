...
test('create a Voie / with not included nomAlt', async t => {
  const _id = new mongo.ObjectId()
  await mongo.db.collection('bases_locales').insertOne({
    _id,
  })
  ...
  const voie = await Voie.create(baseLocale, {nom: 'foo  oo', nomAlt: {gua: 'Lapwent'}})
