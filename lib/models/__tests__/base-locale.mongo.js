...
test('Delete a demo BaseLocale', async t => {
  ...
  await mongo.db.collection('voies').insertOne({_bal: _id, numero: 1})

  await BaseLocale.remove(_id)

  t.falsy(await mongo.db.collection('bases_locale').findOne({_id}))
...
test('remove a BaseLocale', async t => {
  ...
  const voie = await mongo.db.collection('voies').findOne({_bal: _id})
  const numero = await mongo.db.collection('numeros').findOne({_bal: _id})

  t.truthy(baseLocale._deleted)
  t.truthy(voie._deleted)
  t.truthy(numero._deleted)

  t.deepEqual(baseLocale._deleted, baseLocale._updated)
  t.deepEqual(voie._deleted, voie._updated)
  t.deepEqual(numero._deleted, numero._updated)
})
...
test('Count numeros & certified numeros', async t => {
  ...
  await mongo.db.collection('numeros').insertOne({_bal: _id, numero: 1, commune: '55326', certifie: true})
  await mongo.db.collection('numeros').insertOne({_bal: _id, numero: 2, commune: '55326', certifie: false})

  const baseLocale = await BaseLocale.getNumerosCount(_id, '55326')

  t.is(baseLocale.nbNumeros, 2)
  t.is(baseLocale.nbNumerosCertifies, 1)
})
