...
test('create a BaseLocale', async t => {
  ...
  const keys = ['nom', 'status', 'emails', 'commune', 'token', '_updated', '_created', '_id']
...
test('create a BaseLocale / minimal', async t => {
  ...
  const keys = ['nom', 'status', 'emails', 'token', '_updated', '_created', '_id']
  t.true(keys.every(k => k in baseLocale))
  t.is(Object.keys(baseLocale).length, 8)
})
...
test('create a BaseLocale / demo', async t => {
  ...
  const keys = ['nom', 'status', 'token', '_updated', '_created', '_id', 'commune']
  t.true(keys.every(k => k in baseLocale))
...
test('Count numeros & certified numeros', async t => {
  ...
  const baseLocale = await BaseLocale.getCommune(_id, '55326')

  t.is(baseLocale.idBal, _id)
  t.is(baseLocale.codeCommune, '55326')
...
test('batch baselocale numeros', async t => {
  ...
  await BaseLocale.batchUpdateNumeros(idBal, '12345', {certifie: true})
...
test('batch baselocale numeros / invalid certifie', async t => {
  ...
  const error = await t.throwsAsync(() => BaseLocale.batchUpdateNumeros(idBal, '12345', {
    numeros: [],
    certifie: 'foo'
  }))
