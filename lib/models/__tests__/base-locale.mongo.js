...
test('Create BAL from merge communes', async t => {
  ...
  // On s’assure que la BAL produite correspond aux attentes
  ...
  // On s'assure que le total de voies est correspond (21 pour Mont-Bonvillers + 2 pour la BAL de test)
  const voiesCount = await mongo.db.collection('voies').countDocuments({_bal: mongo.parseObjectID(newBAL._id)})
  t.is(voiesCount, 23)

  // On s’assure que tous les champs de la voie son conservés
  ...
  // On s’assure que le toponyme est bien présent
  ...
  // On s’assire que le nombre de numéros correspond (486 pour Mont-Bonvillers + 2 pour la BAL de test)
  ...
  // On s'assure que les numéros on gardait leur voie originel
  const voieNumero = await mongo.db.collection('voies').findOne({_id: mongo.parseObjectID(numero.voie)})
  t.is(voieNumero.nom, 'Voie A')

  // On s’assure que la BAL qui a été fusionné a bien été soft deleted
