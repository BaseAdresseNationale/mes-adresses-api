...
test('create a BaseLocale / demo', async t => {
  ...
  t.is(baseLocale.nom, 'Base locale de Breux-sur-Avre [DEMO]')
  t.deepEqual(baseLocale.communes, ['27115'])
  t.is(baseLocale.status, 'demo')
})

test('create a BaseLocale / invalid commune', t => {
  return t.throwsAsync(() => BaseLocale.createDemo({
    commune: '00000',
    populate: false
  }), {message: 'Code commune inconnu'})
})
...
test('transform to draft', async t => {
  ...
  const baseLocale = await BaseLocale.transformToDraft(_id, {
    nom: 'foo2',
    email: 'me2@domain.co'
  })
...
test('transform to draft / default nom', async t => {
  ...
  t.is(baseLocale.nom, 'Base Locale de Breux-sur-Avre')
  t.deepEqual(baseLocale.emails, ['me2@domain.co'])
  t.is(baseLocale.status, 'draft')
  t.is(Object.keys(baseLocale).length, 8)
})
