...
test('Check if Base Locale Exist', async t => {
  ...
  const existingBaseLocale = await BaseLocale.checkIfBALExists('55326', 'living@data.com')
  const notExitingBaseLocale = await BaseLocale.checkIfBALExists('55326', 'fetching@data.com')

  t.is(existingBaseLocale[0].emails, undefined)
  t.is(existingBaseLocale[0].token, undefined)
  t.true(existingBaseLocale[0].communes.includes('55326'))
  t.is(notExitingBaseLocale.length, 0)
})
