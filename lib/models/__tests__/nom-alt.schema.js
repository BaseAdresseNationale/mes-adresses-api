...
test('not valid payload: too many nomAlt', t => {
  ...
  t.is(error.message, '"value" failed custom validation because Trop de nom (8 maximum)')
})
...
test('not valid payload: disallowed nomAlt', t => {
  ...
  t.is(error.message, '"value" failed custom validation because Valeurs "nomAlt" inattendues : bra.\nValeurs attendues : fra, bre, eus, asw, cos, gyn, rcf ou oci')
})
