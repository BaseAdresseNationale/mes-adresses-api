...
test('not valid payload: too many nomAlt', t => {
  ...
  t.is(error.message, '"value" failed custom validation because Trop de nom (11 maximum)')
})
...
test('not valid payload: disallowed nomAlt', t => {
  ...
  t.is(error.message, '"value" failed custom validation because Valeurs "nomAlt" inattendues : bra. Valeurs attendues : fra, bre, eus, gsw, cos, gcr, gcf, rcf, swb, oci ou cat')
})
