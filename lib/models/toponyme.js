...
async function getNumerosCount(idToponyme) {
  const nbNumeros = await mongo.db.collection('numeros').countDocuments({toponyme: mongo.parseObjectID(idToponyme)})
  const nbNumerosCertifies = await mongo.db.collection('numeros').countDocuments({toponyme: mongo.parseObjectID(idToponyme), certifie: true})
  const isAllCertified = nbNumeros > 0 && nbNumeros === nbNumerosCertifies

  return {nbNumeros, nbNumerosCertifies, isAllCertified}
}

async function getExpandedToponyme(toponyme) {
  const {nbNumeros, nbNumerosCertifies, isAllCertified} = await getNumerosCount(toponyme._id)

  toponyme.nbNumeros = nbNumeros
  toponyme.nbNumerosCertifies = nbNumerosCertifies
  toponyme.isAllCertified = isAllCertified
