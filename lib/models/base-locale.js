...
async function removeSoftDeletedBALsOlderThanOneYear() {
  ...
  await Promise.all(softDeletedBALs.map(async bal => {
    mongo.db.collection('bases_locales').deleteOne({_id: bal._id})
    cleanContent(bal._id)
  }))

  console.log(`\n${softDeletedBALs.length} BALs supprimées définitivement\n`)
}
