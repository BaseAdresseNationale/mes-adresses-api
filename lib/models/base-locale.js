...
async function publishedBasesLocalesStats() {
  ...
  const nbVoies = await mongo.db.collection('voies').find({_bal: {$in: publishedBasesLocalesIds.map(({_id}) => mongo.parseObjectID(_id))}}).count()
  const nbLieuxDits = await mongo.db.collection('toponymes').find({_bal: {$in: publishedBasesLocalesIds.map(({_id}) => mongo.parseObjectID(_id))}}).count()
  const nbNumerosCertifies = await mongo.db.collection('numeros').find({_bal: {$in: publishedBasesLocalesIds.map(({_id}) => mongo.parseObjectID(_id))}, certifie: true}).count()
...
async function expandModel(baseLocale) {
  ...
  nbVoies = await mongo.db.collection('voies').find({_bal: mongo.parseObjectID(baseLocale._id)}).count()
  nbLieuxDits = await mongo.db.collection('toponymes').find({_bal: mongo.parseObjectID(baseLocale._id)}).count()
  nbNumerosCertifies = await mongo.db.collection('numeros').find({_bal: mongo.parseObjectID(baseLocale._id), certifie: true}).count()
