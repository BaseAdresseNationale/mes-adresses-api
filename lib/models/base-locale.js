...
async function checkPublishedBAL(codeCommune, userEmail) {
  if (codeCommune) {
    const publishedBaseLocale = await mongo.db.collection('bases_locales')
      .find({communes: codeCommune, status: 'published'}).sort({_updated: -1}).toArray()

    if (publishedBaseLocale.length > 0) {
      if (userEmail && publishedBaseLocale[0].emails.includes(userEmail)) {
        return {...filterSensitiveFields(publishedBaseLocale[0]), isOwner: true}
      }

      return {...filterSensitiveFields(publishedBaseLocale[0]), isOwner: false}
    }
  }
