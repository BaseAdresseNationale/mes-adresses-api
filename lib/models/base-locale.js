...
const {getNomCommune} = require('../util/cog')
...
const createDemoSchema = Joi.object().keys({
  commune: Joi.string().min(3).max(5).required(),
...
async function createDemo(payload) {
  ...
  if (!communes.some(c => c.code === commune && ['commune-actuelle', 'arrondissement-municipal'].includes(c.type))) {
    throw new Error('Code commune inconnu')
  }

  const nomCommune = await getNomCommune(commune)

  const baseLocale = {
    token: generateBase62String(20),
    communes: [commune],
    nom: `Base locale de ${nomCommune} [DEMO]`,
    status: 'demo'
  }
...
async function transformToDraft(id, payload) {
  const changes = validPayload(payload, transformToDraftSchema)
  const currentBaseLocale = await mongo.db.collection('bases_locales').findOne({_id: mongo.parseObjectID(id)})
  let nomCommune

  if (!changes.nom) {
    const [codeCommune] = currentBaseLocale.communes
    nomCommune = await getNomCommune(codeCommune)
  }

  const baseLocaleChanges = {
    status: 'draft',
    emails: [changes.email],
    nom: changes.nom || `Base Locale de ${nomCommune}`
  }
  mongo.decorateModification(baseLocaleChanges)
  ...
  const email = createBalCreationNotificationEmail({baseLocale: baseLocaleChanges})
  await sendMail(email, baseLocaleChanges.emails)
