...
async function checkIfBALExists(codeCommune, userEmail) {
  if (codeCommune && userEmail) {
    const exisitingBasesLocales = await mongo.db.collection('bases_locales')
      .find({communes: codeCommune, emails: userEmail}).sort({_updated: -1}).toArray()

    const filteredExistingBasesLocales = exisitingBasesLocales.map(bal => filterSensitiveFields(bal))

    return filteredExistingBasesLocales
  }

  throw new Error('Code commune & email obligatoires')
}
