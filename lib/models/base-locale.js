...
async function addHabilitation(baseLocaleId, habilitation) {
  await mongo.db.collection('bases_locales').updateOne(
...
async function updateHabilitation(baseLocale, habilitation) {
  await mongo.db.collection('bases_locales').updateOne(
  ...
  if (habilitation.status === 'accepted' && baseLocale.habilitation.status !== 'accepted' && habilitation.emailCommune) {
