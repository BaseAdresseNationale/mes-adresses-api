...
const {omit, uniqBy, difference, orderBy} = require('lodash')
...
async function checkPublishedBAL(codeCommune, userEmail) {
  if (codeCommune && userEmail) {
    const publishedBaseLocale = await mongo.db.collection('bases_locales').find({communes: codeCommune, status: 'published'}).toArray()
    const orderedPublishedBaseLocale = orderBy(publishedBaseLocale, '_updated', 'desc')

    if (orderedPublishedBaseLocale.length > 0) {
      if (orderedPublishedBaseLocale[0].emails.includes(userEmail)) {
        return {...orderedPublishedBaseLocale[0], isOwner: true}
      }

      return {...orderedPublishedBaseLocale[0], isOwner: false}
    }
  }
