...
async function groupBALByCommune() {
  ...
  const groupByCommune = () => {
    return results.reduce((acc, obj) => {
      if (Object.keys(acc).includes(obj.communes)) {
        return acc
      }

      acc[obj.communes] = results.filter(({communes}) => communes === obj.communes)
      return acc
    }, {})
  }

  return groupByCommune()
}
