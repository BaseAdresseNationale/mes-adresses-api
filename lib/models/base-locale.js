...
async function fetchByCommunes(codesCommunes) {
  const communes = await mongo.db.collection('communes').find({code: {$in: codesCommunes}}).toArray()
  const basesLocales = await mongo.db.collection('bases_locales').find({_id: {$in: communes.map(({_bal}) => _bal)}}).toArray()
...
async function removeCommune(id, codeCommune) {
  ...
  const {value} = await mongo.db.collection('communes').findOneAndDelete({
