...
const {voiesToPointGeoJSON, numerosToGeoJSON, voiesToLineGeoJSON} = require('../export/geojson')
...
async function getGeoJSON(id) {
  // GET VOIES / NUMEROS / TOPONYME
  const voies = await mongo.db.collection('voies').find({_bal: id, _deleted: null}).toArray()
  const numeros = await mongo.db.collection('numeros').find({_bal: id, 'positions.point': {$exists: true}, _deleted: null}).toArray()
  const toponymes = await mongo.db.collection('toponymes').find({_bal: id, _deleted: null}).toArray()
  // TRANSFORME TO GEOJSON
  const geoJsonPointVoies = voiesToPointGeoJSON(voies, numeros)
  const geoJsonLineVoies = voiesToLineGeoJSON(voies)
  const geoJsonNumeros = numerosToGeoJSON(voies, toponymes, numeros)
  return {
    geoJsonPointVoies,
    geoJsonLineVoies,
    geoJsonNumeros
