...
async function update(id, payload) {
  ...
  const oldBaseLocale = await mongo.db.collection('bases_locales').findOne({_id: mongo.parseObjectID(id)})
  ...
  const newEmails = pullAll(baseLocaleChanges.emails, oldBaseLocale.emails)
  if (newEmails.length > 0) {
    const email = createNewAdminNotificationEmail({baseLocale: value})
    await sendMail(email, newEmails)
  }
