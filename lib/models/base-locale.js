...
async function getBALWithNumerosCount(balId, codeCommune) {
  const baseLocale = await mongo.db.collection('bases_locales').findOne({_id: mongo.parseObjectID(balId)})
  const nbNumeros = await mongo.db.collection('numeros').find({_bal: mongo.parseObjectID(balId), commune: codeCommune}).count()
  const nbNumerosCertifies = await mongo.db.collection('numeros').find({_bal: mongo.parseObjectID(balId), commune: codeCommune, certifie: true}).count()

  return {...baseLocale, nbNumeros, nbNumerosCertifies}
}
