...
async function updateNumerosByBatch(idBal, codeCommune, changes) {
  const now = new Date()
  const {certifie} = changes
  const voiesBeforeUpdate = await mongo.db.collection('numeros').distinct('voie', {_bal: mongo.parseObjectID(idBal), commune: codeCommune, certifie: {$eq: certifie}})
  ...
  if (modifiedCount) {
    const voiesAfterUpdate = await mongo.db.collection('numeros').distinct('voie', {_bal: mongo.parseObjectID(idBal), commune: codeCommune, certifie: {$eq: certifie}})
    const modifiedVoies = difference(voiesAfterUpdate, voiesBeforeUpdate)
    const formattedVoieIds = mongo.parseObjectIDList(modifiedVoies)
