...
async function update(id, payload) {
  ...
  if (currentBaseLocale.status === 'published' && baseLocaleChanges.status && baseLocaleChanges.status !== 'published') {
    throw new Error('La base locale est publiée, son statut ne peut plus être changé')
  }
...
async function publishedBasesLocalesStats() {
  const publishedBasesLocalesIds = await mongo.db.collection('bases_locales').find({status: 'published'}, {projection: {_id: 1}}).toArray()

  const nbVoies = await mongo.db.collection('voies').count({_bal: {$in: publishedBasesLocalesIds.map(({_id}) => mongo.parseObjectID(_id))}})
  const nbLieuxDits = await mongo.db.collection('toponymes').count({_bal: {$in: publishedBasesLocalesIds.map(({_id}) => mongo.parseObjectID(_id))}})
  const nbNumeros = await mongo.db.collection('numeros').count({_bal: {$in: publishedBasesLocalesIds.map(({_id}) => mongo.parseObjectID(_id))}})
  const nbNumerosCertifies = await mongo.db.collection('numeros').count({_bal: {$in: publishedBasesLocalesIds.map(({_id}) => mongo.parseObjectID(_id))}, certifie: true})
...
async function expandModel(baseLocale) {
  ...
  nbVoies = await mongo.db.collection('voies').count({_bal: mongo.parseObjectID(baseLocale._id)})
  nbLieuxDits = await mongo.db.collection('toponymes').count({_bal: mongo.parseObjectID(baseLocale._id)})
  nbNumeros = await mongo.db.collection('numeros').count({_bal: mongo.parseObjectID(baseLocale._id)})
  nbNumerosCertifies = await mongo.db.collection('numeros').count({_bal: mongo.parseObjectID(baseLocale._id), certifie: true})
