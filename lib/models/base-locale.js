...
async function findByCommuneAndEmail(codeCommune, userEmail) {
  const foundedBasesLocales = await mongo.db.collection('bases_locales')
    .find({communes: codeCommune, emails: userEmail}).sort({_updated: -1}).toArray()

  const filteredFoundedBasesLocales = foundedBasesLocales.map(bal => filterSensitiveFields(bal))

  return filteredFoundedBasesLocales
}
