...
const {omit, uniqBy, pullAll} = require('lodash')
...
async function update(id, payload) {
  ...
  if (baseLocaleChanges.emails) {
    const {emails} = await mongo.db.collection('bases_locales').findOne({_id: mongo.parseObjectID(id)})
    newCollaborators = pullAll(baseLocaleChanges.emails, emails)
  }
  ...
  if (newCollaborators) {
    await sendMailToNewCollaborators(newCollaborators, {baseLocale: value})
  }
...
async function sendMailToNewCollaborators(newCollaborators, baseLocale) {
  if (newCollaborators.length > 0) {
    const email = createNewAdminNotificationEmail(baseLocale)
    await sendMail(email, newCollaborators)
  }
}
