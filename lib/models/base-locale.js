...
async function checkPublishedBAL(codeCommune, userEmail) {
  if (codeCommune) {
    const verifiedPublishedBasesLocales = []
    const publishedBasesLocales = await mongo.db.collection('bases_locales')
      .find({communes: codeCommune, status: 'published'}).sort({_updated: -1}).toArray()

    if (publishedBasesLocales.length > 0) {
      publishedBasesLocales.forEach(bal => {
        const isOwner = userEmail && bal.emails.includes(userEmail)
        verifiedPublishedBasesLocales.push({...filterSensitiveFields(bal), isOwner})
      })

      return verifiedPublishedBasesLocales
    }

    return false
  }

  throw new Error('Code commune introuvable')
}
