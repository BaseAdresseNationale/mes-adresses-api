...
async function removeSoftDeletedBALsOlderThanOneYear() {
  ...
  for await (const softDeletedBALsID of softDeletedBALsIDs) {
    mongo.db.collection('bases_locales').deleteOne({_id: softDeletedBALsID})
    cleanContent(softDeletedBALsID)
  }
