...
const createPublicationEmail = require('../emails/bal-publication-notification')
...
const {createHabilitation, sendPinCode, validatePinCode} = require('../util/api-depot')
const Voie = require('./voie')
const Numero = require('./numero')
const Toponyme = require('./toponyme')

function formatHabilitation(habilitation) {
  const fieldsToOmit = ['client', 'codeCommune', 'emailCommune', 'strategy']
  const strategyType = habilitation.strategy ? habilitation.strategy.type : null

  return {...omit(habilitation, ...fieldsToOmit), strategyType}
}
...
async function createBALHabilitation(baseLocaleId, codeCommune) {
  const habilitation = createHabilitation(codeCommune)

  const {value} = await mongo.db.collection('bases_locales').findOneAndUpdate(
    {_id: mongo.parseObjectID(baseLocaleId)},
    {$set: {habilitation: formatHabilitation(habilitation)}},
    {returnOriginal: false}
  )
...
function askAuthentificationCode(habilitationId) {
  return sendPinCode(habilitationId)
}

async function validateAuthentificationCode(baseLocale, code) {
  const habilitation = validatePinCode(baseLocale.habilitation._id, code)
  ...
  if (habilitation.status === 'accepted') {
    ...
    if (habilitation.emailCommune) {
      const email = createPublicationEmail({baseLocale})
      await sendMail(email, [habilitation.emailCommune])
    }
...
module.exports = {
  ...
  getCommune,
  batchUpdateNumeros,
  createBALHabilitation,
  askAuthentificationCode,
  validateAuthentificationCode
}
