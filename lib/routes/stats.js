...
app.route('/bases-locales')
  .get(w(async (req, res) => {
    const stats = await BaseLocale.publishedBasesLocalesStats()
    const {nbVoies, nbLieuxDits, nbNumeros, nbNumerosCertifies} = stats

    res.send({nbVoies, nbLieuxDits, nbNumeros, nbNumerosCertifies})
  }))
...
app.route('/departements/:codeDepartement')
  .get(w(async (req, res) => {
    ...
    const filteredBasesLocales = await Promise.all(
      basesLocales.map(async bal => {
        const expandedBal = await BaseLocale.expandModel(bal)
        return BaseLocale.filterSensitiveFields(expandedBal)
      })
    )

    res.send(filteredBasesLocales)
  }))
