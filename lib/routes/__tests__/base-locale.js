...
test('Check if BAL exists', async t => {
  const _id = new mongo.ObjectID()
  await mongo.db.collection('bases_locales').insertOne({
    _id,
    token: 'coucou',
...
test('Check if BAL exists, no email', async t => {
  const response = await request(getApp())
    .get('/bases-locales/search?codeCommune=55326')

  t.is(response.status, 500)
  t.is(response.body.message, 'Code commune & email obligatoires')
})

test('Check if BAL exists, no commune', async t => {
  const response = await request(getApp())
