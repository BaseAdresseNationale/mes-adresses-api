...
test('GET already published commune BAL', async t => {
  const _id = new mongo.ObjectID()
  await mongo.db.collection('bases_locales').insertOne({
    _id,
    token: 'coucou',
  ...
  const response = await request(getApp())
    .get('/bases-locales/is-published?codeCommune=55326&userEmail=living@data.com')

  t.is(response.status, 201)
  t.is(response.body.length, 1)
  t.is(response.body[0].status, 'published')
  t.true(response.body[0].isOwner)
})

test('GET already published commune BAL, no email', async t => {
  const response = await request(getApp())
    .get('/bases-locales/is-published?codeCommune=55326')

  t.is(response.status, 201)
  t.is(response.body.length, 1)
  t.is(response.body[0].status, 'published')
  t.true(response.body[0].communes.includes('55326'))
  t.is(response.body[0].isOwner, undefined)
})

test('GET already published BAL, no commune', async t => {
  const response = await request(getApp())
    .get('/bases-locales/is-published')

  t.is(response.status, 500)
  t.is(response.body.message, 'Code commune obligatoire')
})

test('GET already published BAL, unpublished commune', async t => {
  const response = await request(getApp())
    .get('/bases-locales/is-published?codeCommune=27115')
