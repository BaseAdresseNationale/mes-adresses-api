...
const INSENSITIVE_FIELDS = ['nom', 'description', '_updated', '_created', '_id', 'communes']
...
test.serial('get all BaseLocale', async t => {
  await mongo.db.collection('bases_locales').remove({})
  ...
  t.is(status, 200)
  t.is(body.length, 2)
  t.true(INSENSITIVE_FIELDS.every(k => k in body[0]))
  t.is(Object.keys(body[0]).length, INSENSITIVE_FIELDS.length)
})
...
test.serial('get a BaseLocal / without admin token', async t => {
  ...
  t.is(status, 200)
  t.true(INSENSITIVE_FIELDS.every(k => k in body))
  t.is(Object.keys(body).length, INSENSITIVE_FIELDS.length)
})
