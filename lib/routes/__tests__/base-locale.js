...
const KEYS = ['nom', 'description', 'emails', 'token', '_updated', '_created', '_id', 'communes', 'status', 'enableComplement']
const SAFE_FIELDS = ['nom', 'description', '_updated', '_created', '_id', 'communes', 'enableComplement']
...
test('export as CSV', async t => {
  mockBan54()
  const {_id} = await BaseLocale.create({emails: ['toto@acme.tld']})
