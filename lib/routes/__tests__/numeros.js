...
const KEYS = ['_id', '_bal', 'commune', 'voie', 'numero', 'numeroComplet', 'positions', 'suffixe', '_created', '_updated', 'comment', 'lieuDit']
...
test.serial('create a numero with lieuDit', async t => {
  const _idVoieA = new mongo.ObjectID()
  const _idVoieB = new mongo.ObjectID()
  const _idBal = new mongo.ObjectID()
  const position = {
  ...
  const {status, body} = await request(getApp())
    .post(`/voies/${_idVoieA}/numeros`)
    .set({Authorization: 'Token coucou'})
    .send({numero: 42, lieuDit: _idVoieB.toHexString()})

  t.is(status, 201)
  t.deepEqual(omit(body, GENERATED_VARS), {
    _bal: _idBal.toHexString(),
    voie: _idVoieA.toHexString(),
    commune: '12345',
    numero: 42,
    numeroComplet: '42',
    suffixe: null,
...
test.serial('create a numero with lieuDit / invalid lieuDit', async t => {
  const _idVoie = new mongo.ObjectID()
  const _idBal = new mongo.ObjectID()
  await mongo.db.collection('bases_locales').insertOne({
    _id: _idBal,
  ...
  const {status, body} = await request(getApp())
    .post(`/voies/${_idVoie}/numeros`)
    .set({Authorization: 'Token coucou'})
    .send({numero: 42, lieuDit: 'invalid lieu-dit'})
  ...
  t.deepEqual(body, {
    ...
    message: 'Invalid payload',
    validation: {
      lieuDit: ['"lieuDit" failed custom validation because Argument passed in must be a single String of 12 bytes or a string of 24 hex characters']
    }
  })
})

test.serial('create a numero with lieuDit / invalid payload', async t => {
  const _idVoieA = new mongo.ObjectID()
  const _idVoieB = new mongo.ObjectID()
  const _idBal = new mongo.ObjectID()
  await mongo.db.collection('bases_locales').insertOne({
