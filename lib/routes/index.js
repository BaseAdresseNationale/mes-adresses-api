...
app.route('/bases-locales/:baseLocaleId/geojson')
  .get(w(async (req, res) => {
    ...
    const tiles = {
      numeros: geojsonvt(geoJsonCache.pointNumeros, options).getTile(z, x, y),
      voies: geojsonvt(geoJsonCache.pointVoies, options).getTile(z, x, y),
      'voies-traces': geojsonvt(geoJsonCache.lineVoies, options).getTile(z, x, y),
    }
