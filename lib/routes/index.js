...
app.route('/bases-locales/search')
  .get(w(async (req, res) => {
    if (!req.query.codeCommune || !req.query.userEmail) {
      res.status(400).send({message: 'Code commune & email obligatoires'})
    }

    const exisitingBasesLocales = await BaseLocale.findByCommuneAndEmail(req.query.codeCommune, req.query.userEmail)
    res.status(200).send(exisitingBasesLocales)
  }))
