...
const {expandModelWithNumerosCount} = require('../util/models')
...
app.route('/bases-locales')
  .get(w(async (req, res) => {
    ...
    const basesLocalesExpanded = await Promise.all(
      basesLocales.map(baseLocale => expandModelWithNumerosCount(baseLocale, '_bal'))
    )
  ...
  .post(w(async (req, res) => {
    ...
    const baseLocaleExpanded = await expandModelWithNumerosCount(baseLocale, '_bal')
...
app.route('/bases-locales/:baseLocaleId')
  .get(w(async (req, res) => {
    const baseLocale = await expandModelWithNumerosCount(req.baseLocale, '_bal')
...
app.route('/bases-locales/:baseLocaleId/toponymes')
  .post(ensureIsAdmin, w(async (req, res) => {
    ...
    const toponymeExpanded = await expandModelWithNumerosCount(toponyme, 'toponyme')
  ...
  .get(w(async (req, res) => {
    ...
    const toponymesExpanded = await Promise.all(
      toponymes.map(toponyme => expandModelWithNumerosCount(toponyme, 'toponyme'))
    )
...
app.route('/voies/:voieId')
  .get(w(async (req, res) => {
    const voie = await expandModelWithNumerosCount(req.voie, 'voie')
    res.send(voie)
  }))
  .put(ensureIsAdmin, w(async (req, res) => {
...
app.route('/toponymes/:toponymeId')
  .get(w(async (req, res) => {
    const toponyme = await expandModelWithNumerosCount(req.toponyme, 'toponyme')
    res.send(toponyme)
  }))
  .put(ensureIsAdmin, w(async (req, res) => {
