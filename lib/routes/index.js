...
app.route('/bases-locales/:baseLocaleId/upload')
  .post(ensureIsAdmin, bodyParser.raw({limit: '50mb', type: 'text/csv'}), w(async (req, res) => {
    ...
    const result = await BaseLocale.importFile(req.baseLocale._id, req.body, req.baseLocale.commune)
    res.send(result)
  }))
...
app.route('/bases-locales/:baseLocaleId/geojson')
  .get(w(async (req, res) => {
    const geojsonStream = await BaseLocale.streamToGeoJSON(req.baseLocale._id, req.baseLocale.commune)
    res.type('json')
...
app.route('/bases-locales/:baseLocaleId/populate')
  .post(ensureIsAdmin, w(async (req, res) => {
    const baseLocale = await BaseLocale.populateCommune(req.baseLocale._id, req.baseLocale.commune)
    res.send(baseLocale)
  }))
...
app.route('/bases-locales/:baseLocaleId/voies')
  .post(ensureIsAdmin, w(async (req, res) => {
    const voie = await Voie.create(req.baseLocale._id, req.baseLocale.commune, req.body)
    res.status(201).send(voie)
  }))
  .get(w(async (req, res) => {
    const voies = await Voie.fetchAll(req.baseLocale._id, req.baseLocale.commune)
    res.send(voies)
  }))

app.route('/bases-locales/:baseLocaleId/toponymes')
  .post(ensureIsAdmin, w(async (req, res) => {
    const toponyme = await Toponyme.create(req.baseLocale._id, req.baseLocale.commune, req.body)
    res.status(201).send(toponyme)
  }))
  .get(w(async (req, res) => {
    const toponymes = await Toponyme.fetchAll(req.baseLocale._id, req.baseLocale.commune)
    res.send(toponymes)
  }))

app.route('/bases-locales/:baseLocaleId/parcelles')
  .get(w(async (req, res) => {
    const parcelles = await BaseLocale.getAssignedParcelles(req.baseLocale._id, req.baseLocale.commune)
    res.send(parcelles)
  }))

app.route('/bases-locales/:baseLocaleId/batch')
  .post(ensureIsAdmin, w(async (req, res) => {
    const batchUpdate = await BaseLocale.batchUpdateNumeros(req.baseLocale._id, req.baseLocale.commune, req.body)
    res.send(batchUpdate)
  }))
