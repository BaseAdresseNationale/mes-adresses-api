...
app.route('/bases-locales/:baseLocaleId/tiles/:z/:x/:y.pbf')
  .get(w(async (req, res) => {
    ...
    const geoJsonCache = await useCache('tile-geojson-vt-' + req.baseLocale._id, 300, async () => {
      const geoJson = await BaseLocale.getGeoJSON(req.baseLocale._id)
      return geoJson
    })

    const tiles = {
      numeros: geojsonvt(geoJsonCache.geoJsonNumeros, options).getTile(z, x, y),
      voies: geojsonvt(geoJsonCache.geoJsonPointVoies, options).getTile(z, x, y),
      'voies-traces': geojsonvt(geoJsonCache.geoJsonLineVoies, options).getTile(z, x, y),
    }
