...
const {createHabilitation, sendPinCode, validatePinCode} = require('../habilitations')
...
app.route('/bases-locales/:baseLocaleId/communes/:codeCommune/authentication')
  .post(ensureIsAdmin, w(async (req, res) => {
    if (req.baseLocale.habilitation) {
      const now = new Date()
      const expiresAt = new Date(req.baseLocale.habilitation.expiresAt)

      if (req.baseLocale.habilitation.status === 'accepted' && expiresAt > now) {
        return res.status(403).send({code: 403, message: 'Cette Base Adresse Locale possède déjà une habilitation'})
      }
    }

    const habilitation = await createHabilitation(req.params.codeCommune)
    await BaseLocale.addHabilitation(req.baseLocale._id, habilitation)
    res.send(habilitation)
  }))

app.route('/bases-locales/:baseLocaleId/communes/:codeCommune/authentication/email/send-pin-code')
  .post(ensureIsAdmin, w(async (req, res) => {
    if (!req.baseLocale.habilitation) {
      return res.status(403).send({code: 403, message: 'Aucune demande d’habilitation trouvée'})
    }

    if (req.baseLocale.habilitation.status !== 'pending') {
      return res.status(403).send({code: 403, message: 'Aucune demande d’habilitation en attente'})
    }

    const {body} = await sendPinCode(req.baseLocale.habilitation._id)
    return res.status(body.code).send({code: body.code, message: body.message})
  }))

app.route('/bases-locales/:baseLocaleId/communes/:codeCommune/authentication/email/validate-pin-code')
  .post(ensureIsAdmin, w(async (req, res) => {
    if (!req.body.code) {
      return res.status(403).send({code: 403, message: 'code est un champs obligatoire'})
    }
    ...
    try {
      const habilitation = await validatePinCode(req.baseLocale.habilitation._id, req.body.code)
      await BaseLocale.updateHabilitation(req.baseLocale, habilitation)
      res.send(habilitation)
    } catch (error) {
      if (error.response) {
      ...
      return res.status(403).send({code: 403, message: error.message})
    }
  }))
