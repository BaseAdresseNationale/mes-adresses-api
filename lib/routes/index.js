...
app.route('/bases-locales')
  .get(w(async (req, res) => {
    ...
    const basesLocalesExpanded = await Promise.all(
      basesLocales.map(baseLocale => BaseLocale.getExpandedBAL(baseLocale))
    )
  ...
  .post(w(async (req, res) => {
    ...
    const baseLocaleExpanded = await BaseLocale.getExpandedBAL(baseLocale)
...
app.route('/bases-locales/:baseLocaleId')
  .get(w(async (req, res) => {
    const baseLocale = await BaseLocale.getExpandedBAL(req.baseLocale)
...
app.route('/bases-locales/:baseLocaleId/toponymes')
  .post(ensureIsAdmin, w(async (req, res) => {
    ...
    const toponymeExpanded = await Toponyme.getExpandedToponyme(toponyme)
  ...
  .get(w(async (req, res) => {
    ...
    const toponymesExpanded = await Promise.all(
      toponymes.map(toponyme => Toponyme.getExpandedToponyme(toponyme))
    )
...
app.route('/voies/:voieId')
  .get(w(async (req, res) => {
    const voie = await Voie.getExpandedVoie(req.voie)
    res.send(voie)
  }))
  .put(ensureIsAdmin, w(async (req, res) => {
...
app.route('/toponymes/:toponymeId')
  .get(w(async (req, res) => {
    const toponyme = await Toponyme.getExpandedToponyme(req.toponyme)
    res.send(toponyme)
  }))
  .put(ensureIsAdmin, w(async (req, res) => {
