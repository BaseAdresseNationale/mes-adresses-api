...
app.route('/bases-locales/:baseLocaleId/numeros/batch')
  .put(ensureIsAdmin, w(async (req, res) => {
    const modifiedCount = await Numero.updateNumerosByBatch(req.baseLocale._id, req.body)
    res.send(modifiedCount)
  }))

app.route('/bases-locales/:baseLocaleId/communes/:codeCommune')
  .get(w(async (req, res) => {
    const commune = await BaseLocale.getCommune(req.baseLocale._id, req.params.codeCommune)
    res.send(commune)
  }))
  .put(ensureIsAdmin, w(async (req, res) => {
...
app.route('/bases-locales/:baseLocaleId/communes/:codeCommune/batch')
  .put(ensureIsAdmin, w(async (req, res) => {
    const modifiedCount = await BaseLocale.updateNumerosByBatch(req.baseLocale._id, req.params.codeCommune, req.body)
    res.send(modifiedCount)
  }))
...
app.route('/voies/:voieId/batch')
  .put(ensureIsAdmin, w(async (req, res) => {
    const modifiedCount = await Voie.updateNumerosByBatch(req.voie._id, req.body)
    res.send(modifiedCount)
  }))
