...
app.route('/bases-locales/:baseLocaleId/communes/:codeCommune/authentification')
  .post(ensureIsAdmin, w(async (req, res) => {
    ...
    const baseLocale = await BaseLocale.createBALHabilitation(req.baseLocale._id, req.params.codeCommune)
    res.send(baseLocale)
  }))

app.route('/bases-locales/:baseLocaleId/communes/:codeCommune/authentification/email/send-pin-code')
  .post(ensureIsAdmin, w(async (req, res) => {
    try {
      await BaseLocale.askAuthentificationCode(req.baseLocale.habilitation._id)
    } catch (error) {
...
app.route('/bases-locales/:baseLocaleId/communes/:codeCommune/authentification/email/validate-pin-code')
  .post(ensureIsAdmin, w(async (req, res) => {
    try {
      const baseLocale = await BaseLocale.validateAuthentificationCode(req.baseLocale, req.body.code)
      res.send(baseLocale)
    } catch (error) {
      if (error.response) {
