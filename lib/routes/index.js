...
const {getCodesCommunes} = require('../util/geo-api')
...
app.route('/stats/departement/:codeDepartement')
  .get(w(async (req, res) => {
    ...
    const codesCommunes = await getCodesCommunes(codeDepartement)
    const basesLocales = await BaseLocale.fetchByCodesCommunes(codesCommunes)
    res.send(basesLocales.map(bal => BaseLocale.filterSensitiveFields(bal)))
  }))
