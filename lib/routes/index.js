...
app.route('/bases-locales')
  ...
  .post(w(async (req, res) => {
    ...
    const baseLocaleExpanded = await expandModelWithNumeros(baseLocale, '_bal')
...
app.route('/bases-locales/:baseLocaleId')
  .get(w(async (req, res) => {
    const baseLocale = await expandModelWithNumeros(req.baseLocale, '_bal')
...
app.route('/bases-locales/:baseLocaleId/tiles/:z/:x/:y.pbf')
  .get(w(async (req, res) => {
    ...
    const tilesIndex = await useCache('tile-geojson-vt-' + req.baseLocale._id, 300, async () => {
      const {geoJsonNumeros, geoJsonPointVoies, geoJsonLineVoies} = await BaseLocale.getGeoJSON(req.baseLocale._id)
      return {
        numeros: geojsonvt(geoJsonNumeros, options),
        pointVoies: geojsonvt(geoJsonPointVoies, options),
        lineVoies: geojsonvt(geoJsonLineVoies, options)
      }
    })

    const tiles = {
      numeros: tilesIndex.numeros.getTile(z, x, y),
      voies: tilesIndex.pointVoies.getTile(z, x, y),
      'voies-traces': tilesIndex.lineVoies.getTile(z, x, y)
    }

    if (!tiles.numeros && !tiles.voies && !tiles['voies-traces']) {
...
app.route('/bases-locales/:baseLocaleId/voies')
  .post(ensureIsAdmin, w(async (req, res) => {
    ...
    const voieExpanded = await expandModelWithNumeros(voie, 'voie')
...
app.route('/bases-locales/:baseLocaleId/toponymes')
  .post(ensureIsAdmin, w(async (req, res) => {
    ...
    const toponymeExpanded = await expandModelWithNumeros(toponyme, 'toponyme')
