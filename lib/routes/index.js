...
app.route('/bases-locales')
  .get(w(async (req, res) => {
    ...
    const basesLocalesExpanded = await Promise.all(
      basesLocales.map(baseLocale => mongo.getNumerosCount(baseLocale, '_bal'))
    )
  ...
  .post(w(async (req, res) => {
    ...
    const baseLocaleExpanded = await mongo.getNumerosCount(baseLocale, '_bal')
...
app.route('/bases-locales/:baseLocaleId')
  .get(w(async (req, res) => {
    const baseLocale = await mongo.getNumerosCount(req.baseLocale, '_bal')
...
app.route('/bases-locales/:baseLocaleId/toponymes')
  .post(ensureIsAdmin, w(async (req, res) => {
    ...
    const toponymeExpanded = await mongo.getNumerosCount(toponyme, 'toponyme')
  ...
  .get(w(async (req, res) => {
    ...
    const toponymesExpanded = await Promise.all(
      toponymes.map(toponyme => mongo.getNumerosCount(toponyme, 'toponyme'))
    )
...
app.route('/voies/:voieId')
  .get(w(async (req, res) => {
    const voie = await mongo.getNumerosCount(req.voie, 'voie')
    res.send(voie)
  }))
  .put(ensureIsAdmin, w(async (req, res) => {
...
app.route('/toponymes/:toponymeId')
  .get(w(async (req, res) => {
    const toponyme = await mongo.getNumerosCount(req.toponyme, 'toponyme')
    res.send(toponyme)
  }))
  .put(ensureIsAdmin, w(async (req, res) => {
