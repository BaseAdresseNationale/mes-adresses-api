...
app.route('/bases-locales/:baseLocaleId')
  ...
  .put(ensureIsAdmin, w(async (req, res) => {
    if (req.baseLocale.status === 'demo') {
      return res.status(403).send({code: 403, message: 'Modification d’une Base Adresse Locale de démonstration non autorisée'})
    }
...
app.route('/bases-locales/:baseLocaleId/transform-to-draft')
  .post(ensureIsAdmin, w(async (req, res) => {
    ...
    return res.status(403).send({code: 403, message: 'La Base Adresse Locale n’est pas une Base Adresse Locale de démonstration'})
  }))
