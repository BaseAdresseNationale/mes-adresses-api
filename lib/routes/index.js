...
const {createHabilitation, sendPinCode, validatePinCode, prepareHabilitation} = require('../habilitations')
...
function ensurePendingHabilitation(req, res, next) {
  if (!req.baseLocale.habilitation) {
    return res.status(412).send({code: 412, message: 'Aucune demande d’habilitation trouvée'})
  }
...
app.route('/bases-locales/:baseLocaleId/authentication')
  .post(ensureIsAdmin, w(async (req, res) => {
    if (req.baseLocale.habilitation) {
      const now = new Date()
      const {status, expiresAt} = req.baseLocale.habilitation

      if (status === 'accepted' && expiresAt > now) {
    ...
    if (req.baseLocale.communes.length > 1) {
      return res.status(412).send({code: 412, message: 'Pour obtenir une habilitation cette BAL doit avoir exactement une commune de rattachement'})
    }

    const habilitation = await createHabilitation(req.baseLocale.communes[0])
    await BaseLocale.updateHabilitation(req.baseLocale._id, habilitation)
    res.send(habilitation)
  }))

app.route('/bases-locales/:baseLocaleId/authentication/email/send-pin-code')
  .post(ensureIsAdmin, ensurePendingHabilitation, w(async (req, res) => {
...
app.route('/bases-locales/:baseLocaleId/authentication/email/validate-pin-code')
  .post(ensureIsAdmin, ensurePendingHabilitation, w(async (req, res) => {
    if (!req.body.code) {
    ...
    try {
      const {body} = await validatePinCode(req.baseLocale.habilitation._id, req.body.code)

      if (body.validated === false) {
        return res.status(200).send({code: 200, ...body})
      }

      const habilitation = prepareHabilitation(body)
      await BaseLocale.updateHabilitation(req.baseLocale._id, habilitation)
      res.send(habilitation)
    } catch (error) {
