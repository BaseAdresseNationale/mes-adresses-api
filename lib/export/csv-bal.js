...
async function exportAsCsv({voies, toponymes, numeros}) {
  ...
  const toponymesWithNumeros = {}
  const rows = []

  numeros.forEach(n => {
    const voieId = n.voie.toHexString()
    const v = voiesIndex[voieId]
    const toponyme = n.toponyme ? toponymes.find((({_id}) => _id.equals(n.toponyme))) : null

    if (toponyme) {
      const toponymeId = n.toponyme.toHexString()
      toponymesWithNumeros[toponymeId] = true
    }
    ...
    if (n.positions && n.positions.length > 0) {
      n.positions.forEach(p => {
        rows.push({
          ...
          _updated: n._updated,
          nomVoie: v.nom,
          complementNomVoie: toponyme ? toponyme.nom : null,
          position: p
        })
      })
    } else {
  ...
  toponymes.filter(t => !(t._id.toHexString() in toponymesWithNumeros)).forEach(t => {
