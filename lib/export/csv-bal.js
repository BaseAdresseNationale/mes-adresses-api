...
async function exportAsCsv({voies, toponymes, numeros}) {
  ...
  const toponymesWithNumeros = new Set()
  const rows = []
  ...
  numeros.forEach(n => {
    ...
    if (n.toponyme) {
      ...
      toponymesWithNumeros.add(toponyme._id.toHexString())
  ...
  toponymes.filter(t => !toponymesWithNumeros.has(t._id.toHexString())).forEach(t => {
