...
function fillVoieProperties(v, numeros = []) {
  return {
    id: v._id.toHexString(),
    type: 'voie',
    idVoie: v._id.toHexString(),
    nomVoie: v.nom,
    nbNumero: numeros.length,
    nbNumeroCertifie: numeros.filter(n => n.certifie).length,
    color: randomColor({
      luminosity: 'dark',
      seed: v._id.toHexString()
    })
...
function voieToPointFeature(v, numeros) {
  ...
  if (numerosFeatures.length > 0) {
    ...
    voieFeature.properties = fillVoieProperties(v, numeros)
