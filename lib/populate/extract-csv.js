...
const {groupBy, deburr, countBy, flatten, union} = require('lodash')
...
function formatHameaux(hameaux) {
  const hameauxGroups = Object.keys(hameaux).map(hameau => groupBy(hameaux[hameau], r => normalizeString(r.lieudit_complement_nom)))
  const formattedHameaux = []

  hameauxGroups.forEach(hameau => {
    Object.values(hameau).forEach(r => {
      const countedToponymes = countBy(r, 'lieudit_complement_nom')
      const sortedToponymes = Object.entries(countedToponymes).sort((a, b) => b[1] - a[1])

      formattedHameaux.push({
        nomToponyme: flatten(sortedToponymes)[0],
        numerosRows: r,
        commune: Object.keys(hameaux)[hameauxGroups.length - 1]
      })
    })
  })
...
function extractVoiesNumeros(normalizedRows) {
  ...
  if (hasHameaux) {
    const hameauxGroupedByCommune = groupBy(hameaux, r => r.cle_interop.slice(0, 5))
    const formattedHameaux = formatHameaux(hameauxGroupedByCommune)
